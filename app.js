...
document.addEventListener("pointerup", e=>{
  ...
  if(k==="="){
    if(mode==="secretDate"){ if(operator==="×"){ X=calc(X,parseFloat(current),"×"); current=String(X); operator=null; } else if(operator==="+" && !waitingForY){ current=String(Z); operator=null; } update(); return; }
    if(operator){ current=String(calc(previous,parseFloat(current),operator)); operator=null; previous=null; update(); return; }
  }
  if(["+","−","×","÷"].includes(k)){
    if(mode==="secretDate"){ const val=parseFloat(current); if(k==="×"){ X=X===null?val:calc(X,val,"×"); operator="×"; waiting=true; } if(k==="+"){ waitingForY=true; Y=""; current="0"; Z=getZ(); fullY=String(Z-X); operator="+"; } return; }
